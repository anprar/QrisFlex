...
export function BrandLogo({ className, href = "/" }: { className?: string; href?: string }) {
  return (
    <Link className={cn("group inline-flex min-w-0 items-center gap-3", className)} href={href}>
      ...
      <span className="min-w-0">
        <span className="font-display block truncate text-lg font-bold leading-none tracking-tight">QrisFlex</span>
        <span className="block truncate text-xs uppercase tracking-[0.18em] text-muted-foreground sm:tracking-[0.28em]">Static to Dynamic</span>
      </span>
    </Link>
  );
}
